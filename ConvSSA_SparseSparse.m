function [X, res] = ConvSSA_SparseSparse(D, S, gamma1,gamma2, opts)
% CONVOLUTIONAL SIMULTANEOUS SPARSE APPROXIMATION (with sparse-sparse structure: nonzero entries are not exactly the same) 
% (both L1 and L2,1 regularizations are used with parameters gamma1 and gamma2 respectively)
%
% Inputs:
...
%   gamma1:           element sparsity regularization (L1-norm)
%   gamma2:           row sparsity regularization (L2,1-norm)
%   (optionals:)
...
while itr<=MaxIter && (r > epri || s > edua)
    ...
    X = prox_L21(sfthrsh(Yr+U, gamma1/rho), gamma2/rho); % X update: shrinkage
    ...
    L11 = sum(vec(abs(X)));
    fval = rPow/2 + gamma1*L11 + gamma2*L12; % functional value
...
function Y = prox_L21(X, kappa) % proximal operator for L2,1-norm 
Y = zeros(size(X));
for i = 1:size(X,5)
